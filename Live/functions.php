...
// $url = 'https://www.xtrader.co.uk';
// $data = [
//     'Type' => 'ORDEREXTOC',
//     'testingmode' => true,
//     'VendorCode' => '50576',
//     'VendorTxCode' => 'IDWEB-50576-TESTMODE-5628-20100304022456',
//     'VenderPass' => 'Flicker01',
//     'VenderSite' => 'https://hunnysbunny.com',
//     'Venderserial' => 'J1amRk',
...
//     'deliveryCountry' => 'GB',
//     'ProductCodes' => 'MODEL&C13003{Size}X Large:1'
// ];
...
function custom_related_products_label( $label ) {
    ...
    $label = __( 'You may also like Products', 'your-text-domain' );
    return $label;
}
...
function bubblechaz_create_terms_on_submission1() {
	if(isset($_GET['submit_type']) && $_GET['submit_type']=='xtrade-loop-data'){
		...
		global $wpdb;
			...
			$retrieve_data = $wpdb->get_results( "select * from wp_xt_data ORDER BY RAND()" );
				if ( !$retrieve_data ) {
					echo 'Data source file not found please provide the correct url.';
					die;
				}
				$ii = 0;
					foreach($retrieve_data as $key=>$productData ){
						...
						$post_id = wc_get_product_id_by_sku( "$productData->MODEL" );
						if(!$post_id){
							$post_id = wc_get_product_id_by_sku( "$productData->EAN" );
						}
						...
						if(!$post_id){
							...
							$result = $wpdb->delete(
								'wp_xt_data',
								array( 'id' => $productData->id ),
								array( '%d' ) // The ID column type, adjust as necessary
							);
							if($ii>=19){
